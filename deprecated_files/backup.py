...
#---------------- Page Setup ----------------#
page_title = "Human Evaluation by Shayom"
page_icon = "👨‍"
...
id_ref = db.collection("All-IDs").document("Shayom-Gold-100-ids")
...
def fetch_trial_data(index, gen_model):
    ...
    if doc.exists:
        ...
        if 'human-response' in model_data:
...
if st.session_state.index <= last_saved_id_index:
    st.warning(f"You are going back to a previously saved trial with trial id {all_gold_ids[st.session_state.index]}. \
               Please review the response before submitting.")
